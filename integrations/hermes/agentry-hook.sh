#!/bin/sh
# Agentry integration hook for Hermes Agent.
# AGENTRY_INTEGRATION_ID=hermes
# AGENTRY_INTEGRATION_VERSION=1
#
# Hermes runs shell hooks declared in ~/.hermes/config.yaml as subprocesses,
# piping a JSON payload to stdin (`hook_event_name`, `tool_name`, `tool_input`,
# `session_id`, `cwd`, `extra`). The real event is carried in the payload, NOT
# in argv, so we dispatch on the payload. Registered events: on_session_start
# (session id), pre_tool_call/post_tool_call (timeline + file paths + activity),
# on_session_end (idle).
...
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/agentry-hermes-hook.XXXXXX")" || exit 0
...
event_name = str(hook.get("hook_event_name") or hook.get("type") or "")
...
def send(method, extra):
    params = {
        "pane_id": pane_id,
        "source": "agentry:hermes",
        "agent": "hermes",
        "seq": time.time_ns(),
    }
    params.update(extra)
    req = {
        "id": f"agentry:hermes:{int(time.time() * 1000)}:{random.randrange(1_000_000):06d}",
        "method": method,
        "params": params,
    }
    try:
...
# Activity state. Hermes emits snake_case hook_event_name values. There is no
# "awaiting input" lifecycle event, so only working/idle toggle here.
state = None
if event_name in ("pre_tool_call", "post_tool_call"):
    state = "working"
elif event_name in ("on_session_end", "subagent_stop"):
    state = "idle"
