<script lang="ts">
    ...
    async function bootstrap() {
        try {
            ...
            settings.set({
                defaultProfileId:
                    (s.default_profile_id as string | null) ?? null,
                maxConcurrentSessions:
                    (s.max_concurrent_sessions as number) ?? 8,
    ...
    async function bindListeners() {
        ...
        unlisteners.push(
            await onProjectCreated((e) => {
                ...
                if (!$ui.activeProjectId)
                    ui.update((u) => ({ ...u, activeProjectId: e.project_id }));
            }),
        );
        unlisteners.push(
            await onSessionStarted((e) => {
                // Was this session already in our store? If so this is a
                // resume/re-emit, not a brand-new start — it must NOT consume a
                // pending-focus credit meant for a freshly created session.
                const isNew = !$sessions.has(e.session_id);
                upsertSession({
                    id: e.session_id,
                ...
                // Auto-focus rules:
                //  1. User explicitly started this session (New / duplicate /
                //     restart) — always focus it, even over another open session.
                //  2. No session focused yet — focus the new one.
                //  3. The just-started session IS the focused one (resume case):
                //     re-pick so the terminal clears its stale scrollback before
                //     the agent re-renders its history.
                const userStarted = isNew && takePendingFocus();
                if (
                    userStarted ||
                    !$ui.focusedSessionId ||
                    $ui.focusedSessionId === e.session_id
                ) {
        ...
        unlisteners.push(
            await onAgentOutput((e) => {
                ...
                if (
                    cur &&
                    (cur.status === "starting" || cur.status === "queued")
                ) {
                    updateSession(e.session_id, { status: "running" });
                }
                if (
                    cur &&
                    (cur.status === "finished" || cur.status === "failed")
                ) {
                    return;
                }
                ...
                if (replayingSessionId === e.session_id) {
                    ...
                    if (!q) {
                        q = [];
                        pendingChunks.set(e.session_id, q);
                    }
                    q.push(bytes);
                    return;
                }
        ...
        unlisteners.push(
            await onSessionFinished((e) => {
                ...
                updateSession(e.session_id, {
                    ...
                    failReason:
                        e.exit_code === 0 ? null : `exit ${e.exit_code}`,
                });
    ...
    function isActive(
        s: import("$lib/types").SessionState | undefined,
    ): boolean {
        if (!s) return false;
        return (
            s.status === "running" ||
    ...
    async function quickStartDefault() {
        ...
        if (!projId) {
            ...
            if (!onboarded) openOnboarding();
                ...
                toasts.info(
                    "No project selected",
                    "Use the activity bar to create one.",
                );
            return;
        }
    ...
    // Keep the file-explorer tree rooted at the active project's path. The
    // daemon's fs_access guard already allows project roots, so this is the
    // natural browse root. Switching projects re-roots and resets the tree.
    $effect(() => {
        explorer.setRoot(activeProject?.path ?? null);
    });
    ...
    >
        <div class="flex flex-1 min-w-0 overflow-hidden">
            ...
            <div class="flex flex-col flex-1 min-w-0">
                ...
                {#if bootstrapError}
                    <div
                        class="px-3 py-2 text-xs text-destructive-foreground bg-destructive"
                    >
                ...
                <div class="flex flex-1 overflow-hidden">
                    {#if $ui.view === "terminal"}
                        {#snippet sidebarPane()}
                            <div
                                class="h-full w-full border-r border-border-strong overflow-hidden flex flex-col"
                            >
                                <!-- Sidebar mode switch: Sessions ⇄ Files -->
                                <div
                                    class="flex items-center gap-0.5 border-b border-border-strong px-1 py-1 flex-shrink-0"
                                >
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        class={cn(
                                            "flex-1 h-7 gap-1.5 text-xs rounded",
                                            explorer.mode === "sessions"
                                                ? "bg-secondary text-foreground"
                                                : "text-muted-foreground",
                                        )}
                                        onclick={() =>
                                            explorer.setMode("sessions")}
                                    >
                                        <ListIcon size={13} /> Sessions
                                    </Button>
                                    <Button
                                        variant="ghost"
                                        size="sm"
                        ...
                        {#snippet mainArea()}
                            <div class="relative h-full w-full overflow-hidden">
                                ...
                                {#if $ui.focusedSessionId}
                                    ...
                                    {:else}
                                        ...
                                        >
                                            ...
                                            {#snippet right()}
                                                <div
                                                    class="h-full w-full border-l border-border overflow-hidden"
                                                >
                                                    <Inspector />
                                                </div>
                                            {/snippet}
                                ...
                                {:else}
                                    <div
                                        class="flex flex-col items-center justify-center h-full gap-5 p-8 text-center"
                                    >
                                        <div
                                            class="flex flex-col items-center gap-3"
                                        >
                                            <div
                                                class="grid place-items-center w-14 h-14 rounded-2xl bg-secondary/60 border border-border text-muted-foreground"
                                            >
                                                <SquareTerminal
                                                    size={26}
                                                    class="text-accent"
                                        ...
                                        <div
                                            class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-xl"
                                        >
                                            <Button
                                                variant="outline"
                                                class="bg-card border-border hover:border-accent/60 hover:bg-accent/5 rounded-lg p-4 h-auto flex-col items-start text-left gap-0 group transition-colors"
                                                onclick={() =>
                                                    quickStartDefault()}
                                            >
                                                <Plus
                                                ...
                                                <kbd
                                                    class="mt-2 inline-block text-[10px] font-mono text-muted-foreground"
                                                    >{fmtChord([
                                                        "mod",
                                                        "t",
                                                    ])}</kbd
                                            ...
                                            <Button
                                                variant="outline"
                                                class="bg-card border-border hover:border-accent/60 hover:bg-accent/5 rounded-lg p-4 h-auto flex-col items-start text-left gap-0 transition-colors"
                                                onclick={() => openPalette()}
                                            >
                                                <Command
                                                    size={18}
                                                    class="text-gruvbox-aqua mb-2"
                                                />
                                                <div
                                                    class="text-sm font-medium"
                                                >
