...
<script lang="ts">
	...
	let selProjectLabel = $derived(
		projectList.find((p) => p.id === selProjectId)?.name ?? '— Select project —'
	);
...
<Dialog.Root open onOpenChange={(v) => { if (!v) closeOnboarding(); }}>
	<Dialog.Content class="w-full max-w-lg" onkeydown={onKeydown}>
		...
		<div class="space-y-4">
			<!-- Project -->
			<div class="block">
				<Label class="mb-1 block text-xs text-muted-foreground">Project <span class="text-destructive">*</span></Label>
				<Select.Root type="single" bind:value={selProjectId}>
					<Select.Trigger class="w-full">{selProjectLabel}</Select.Trigger>
					<Select.Content>
						{#each projectList as p (p.id)}
							<Select.Item value={p.id}>{p.name} <span class="text-muted-foreground font-mono">{p.path}</span></Select.Item>
						{/each}
					</Select.Content>
				</Select.Root>
			...
			<!-- Profile -->
			<div class="block">
				<Label class="mb-1 block text-xs text-muted-foreground">Profile <span class="text-destructive">*</span></Label>
				<Select.Root type="single" bind:value={selProfileId}>
					<Select.Trigger class="w-full">{selProfileLabel}</Select.Trigger>
					<Select.Content>
						{#each profileList as p (p.id)}
							{@const m = agentMeta(p.agent_type)}
							<Select.Item value={p.id}>{p.name} ({m.label})</Select.Item>
						{/each}
					</Select.Content>
				</Select.Root>
			</div>
			...
			<!-- CWD override -->
			<div class="block">
				<Label class="mb-1 block text-xs text-muted-foreground">Working directory <span class="text-muted-foreground/60">(optional — defaults to project path)</span></Label>
			...
			<!-- Initial prompt -->
			<div class="block">
				<Label class="mb-1 block text-xs text-muted-foreground">Initial prompt <span class="text-muted-foreground/60">(optional)</span></Label>
				<Textarea
		...
		<Dialog.Footer class="items-center justify-between sm:justify-between">
			<span class="text-[11px] text-muted-foreground font-mono">{modKey}↩ to start</span>
			<Button
